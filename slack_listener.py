...
def handle_mention(event, say):
    ...
    # Format is typically: "<@U12345> your message here"
...
def handle_message(event, say):
    ...
    - Auto-responds to bot messages containing **test** markers
    """
    ...
    # Auto-responder: Respond to bot messages containing **test** markers
    # This allows Claude to test end-to-end without human intervention
    is_bot_message = bot_id or subtype == "bot_message"
    has_test_marker = "**test" in text.lower()  # Matches both **test** and **test:COMMAND**
    is_auto_response = "🤖 Auto-response:" in text  # Prevent infinite loops

    if is_bot_message and has_test_marker and not is_auto_response:
        import re

        print(f"🤖 Auto-responding to test message: {text[:100]}", file=sys.stderr)

        # Parse test command:
        # Format 1: "Message **test:COMMAND**" -> respond with "COMMAND"
        # Format 2: "Message **test**" -> respond with "test received"
        match = re.search(r'\*\*test:([^*]+)\*\*', text, re.IGNORECASE)

        if match:
            response_text = match.group(1).strip()
        else:
            response_text = "test received"

        # Send response to Claude session (same path as user messages)
        mode = send_response(response_text, thread_ts=thread_ts)

        # Also reply in Slack thread for visibility
        reply_thread_ts = thread_ts or event.get("ts")
        say(
            text=f"🤖 Auto-response: `{response_text}`",
            thread_ts=reply_thread_ts
        )

        print(f"✅ Auto-responded with: {response_text} (via {mode})", file=sys.stderr)
        return

    # Ignore other bot messages (prevents loops)
    if is_bot_message:
        return
